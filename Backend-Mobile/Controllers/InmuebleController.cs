...
public class InmuebleController : ControllerBase{
    ...
    public async Task<ActionResult<IEnumerable<InmuebleResponseDto>>> GetInmueblesBySearch(string search)
    {
        var inmuebles = await _repository.SearchInmuebles(search);
        return Ok(_mapper.Map<IEnumerable<InmuebleResponseDto>>(inmuebles));
    }

    [HttpGet("category/{id}")]
    public async Task<ActionResult<IEnumerable<InmuebleResponseDto>>> GetByCategoryId(int id)
    {
        var inmuebles = await _repository.GetByCategoryId(id);
        return Ok(_mapper.Map<IEnumerable<InmuebleResponseDto>>(inmuebles));
    }

    [HttpGet("trending")]
    public async Task<ActionResult<IEnumerable<InmuebleResponseDto>>> GetTrending()
    {
        var inmuebles = await _repository.GetTrending();
        return Ok(_mapper.Map<IEnumerable<InmuebleResponseDto>>(inmuebles));
    }
